...
// Label returns the direction spelled out for log output. Without it a sync
// reads as ambiguous, since per-entry lines carry IDs but never a direction.
func (d SyncDirection) Label() string {
	if d == SyncDirectionReverse {
...
// DirectionOf reports which way the given reverse flag syncs.
func DirectionOf(reverse bool) SyncDirection {
	if reverse {
		return SyncDirectionReverse
	}
