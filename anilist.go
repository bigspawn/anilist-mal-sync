...
func (c *AnilistClient) GetUserMangaList(ctx context.Context) ([]verniy.MediaListGroup, error) {
	...
	err := retryWithBackoff(ctx, func() error {
		...
		if e != nil {
			return fmt.Errorf("failed to fetch user manga list from AniList: %w", e)
		}
		result = mediaListGroups
		return nil
...
func (c *AnilistClient) UpdateAnimeEntry(
	...
	return retryWithBackoff(ctx, func() error {
		...
		if err != nil {
			return fmt.Errorf("failed to marshal GraphQL request body for anime update: %w", err)
		}
		...
		if len(graphqlResp.Errors) > 0 {
			return fmt.Errorf("AniList GraphQL API returned errors for anime update: %+v", graphqlResp.Errors)
		}
...
func (c *AnilistClient) UpdateMangaEntry(
	...
	return retryWithBackoff(ctx, func() error {
		...
		jsonBody, err := json.Marshal(requestBody)
		if err != nil {
			return fmt.Errorf("failed to marshal GraphQL request body for manga update: %w", err)
		}
		...
		if len(graphqlResp.Errors) > 0 {
			return fmt.Errorf("AniList GraphQL API returned errors for manga update: %+v", graphqlResp.Errors)
		}
...
// GetAnimesByMALID searches AniList for anime matching the given MAL ID
func (c *AnilistClient) GetAnimesByMALID(ctx context.Context, malID int, prefix string) ([]verniy.Media, error) {
	var result []verniy.Media

	err := retryWithBackoff(ctx, func() error {
		page, e := c.c.SearchAnimeWithContext(ctx, verniy.PageParamMedia{IDMAL: malID}, AnilistSearchPage, AnilistSearchPerPage,
			verniy.MediaFieldID,
			verniy.MediaFieldIDMAL,
			verniy.MediaFieldTitle(
				verniy.MediaTitleFieldRomaji,
...
// GetMangasByMALID searches AniList for manga matching the given MAL ID
func (c *AnilistClient) GetMangasByMALID(ctx context.Context, malID int, prefix string) ([]verniy.Media, error) {
	var result []verniy.Media

	err := retryWithBackoff(ctx, func() error {
		page, e := c.c.SearchMangaWithContext(ctx, verniy.PageParamMedia{IDMAL: malID}, AnilistSearchPage, AnilistSearchPerPage,
			verniy.MediaFieldID,
			verniy.MediaFieldIDMAL,
			verniy.MediaFieldTitle(
				verniy.MediaTitleFieldRomaji,
...
func (c *AnilistClient) GetAnimesByName(ctx context.Context, name string, prefix string) ([]verniy.Media, error) {
	...
	err := retryWithBackoff(ctx, func() error {
		page, e := c.c.SearchAnimeWithContext(ctx, verniy.PageParamMedia{Search: name}, AnilistSearchPage, AnilistSearchPerPage,
			verniy.MediaFieldID,
			verniy.MediaFieldIDMAL,
			verniy.MediaFieldTitle(
				verniy.MediaTitleFieldRomaji,
...
func (c *AnilistClient) GetMangasByName(ctx context.Context, name string, prefix string) ([]verniy.Media, error) {
	...
	err := retryWithBackoff(ctx, func() error {
		page, e := c.c.SearchMangaWithContext(ctx, verniy.PageParamMedia{Search: name}, AnilistSearchPage, AnilistSearchPerPage,
			verniy.MediaFieldID,
			verniy.MediaFieldIDMAL,
			verniy.MediaFieldTitle(
				verniy.MediaTitleFieldRomaji,
